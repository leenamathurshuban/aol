              ...
              <div class="card-header">
                  ...
                  <div class="row">
                    ...
                    <div class="col-md-3">
                      ...
                      {{ Form::select('request_type',['inv'=>'PO Invoices','wdinv'=>'Without PO Invoices','bnkRtrn'=>'Internal Transfer','empPay'=>'Employee Pay'],'',['class'=>'form-control custom-select select2','placeholder'=>'Choose type','required'=>true]) }}
              ...
              <div class="card-body">
                ...
                <div class="row">
                    <div class="col-md-12 center-block">
                      @if ($invTot >= $widInvTot && $invTot >= $poTot && $invTot >= $bnkTransTot && $invTot >= $empPayTot)
                         {{$invoiceData->appends($_GET)->links()}}
                      @elseif ($widInvTot >= $invTot && $widInvTot >= $poTot && $widInvTot >= $bnkTransTot && $widInvTot >= $empPayTot)
                         {{$withoutPOinvoiceData->appends($_GET)->links()}}
                      @elseif($poTot >= $invTot && $poTot >= $widInvTot && $poTot >= $bnkTransTot && $poTot >= $empPayTot)
                         {{$poData->appends($_GET)->links()}}
                      @elseif($bnkTransTot >= $invTot && $bnkTransTot >= $widInvTot && $bnkTransTot >= $poTot && $bnkTransTot >= $empPayTot)
                         {{$bnkTransData->appends($_GET)->links()}}
                      @elseif($empPayTot >= $invTot && $empPayTot >= $widInvTot && $empPayTot >= $poTot && $empPayTot >= $bnkTransTot)
