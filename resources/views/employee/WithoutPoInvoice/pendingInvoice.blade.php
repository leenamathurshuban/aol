              ...
              <div class="card-header">
                ...
                {{--  @if(Auth::guard('employee')->user()->role_id==4)
                  <div class="col-sm-12 col-md-1">
                    {{ link_to_route('employee.addWithoutPoInvoice','Add',[],['class'=>'btn btn-block btn-outline-primary','title'=>'Add New']) }}
                  </div>
                  @endif --}}
                ...
                @forelse($data as $result)
                  <tr>
                    ...
                    <td>
                      ...
                      @if(Auth::guard('employee')->user()->role_id==4 && ($result->invoice_status==1 || $result->invoice_status==2))
                            {!! Html::decode(link_to_route('employee.WithoutPoAddItemView','<i class="fas fa-list-alt"></i>Item',[$result->order_id,$currentPage],['class'=>'btn btn-app btn-edit','title'=>'Item Detail'])) !!}

                            {!! Html::decode(link_to_route('employee.editPendingWithoutPoInvoice','<i class="fas fa-edit"></i>Edit',[$result->order_id,$currentPage],['class'=>'btn btn-app btn-edit','title'=>'Edit Invoice'])) !!}
                          
                            {!! Html::decode(link_to_route('employee.removeWithoutPoInvoice','<i class="fas fa-trash"></i>Delete',[$result->order_id],['class'=>'btn btn-app btn-delete','title'=>'Remove','onclick'=>'return removeData()'])) !!}
                      ...
                      @elseif(Auth::guard('employee')->user()->role_id==5 && $result->invoice_status==1)
                        {!! Html::decode(link_to_route('employee.WithoutPoinvoiceStatusApprove','<i class="fas fa-check"></i>Approval',[$result->order_id,$currentPage],['class'=>'btn btn-app btn-edit','title'=>'Approval'])) !!}
