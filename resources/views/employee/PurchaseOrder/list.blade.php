          ...
          <div class="col-12">
            <div class="card">
              ...
              <div class="card-body">
                ...
                @if(\App\PurchaseOrder::chkOrderInvoiceStatus($result->id) >= $result->net_payable)
                  @php $poAppInvTR='poAppInvTR';@endphp
                @endif
                  <tr class="{{$poAppInvTR ?? ''}}">
                    <td>{{ ++$page }}</td>
                    <td>
                   ...
                   <td>
                      {{--!!Form::open(['route'=>['employee.changeActStatus',$result->order_id],'file'=>'true'])!!}
                                                    {!! Form::select('status',\App\PurchaseOrder::status(),[$result->status],['class'=>'form-control custom-select select2','onchange'=>'this.form.submit()','style'=>'width:100%'])!!}             
                                               <span class="text-danger">{{ $errors->first('status')}}</span>               
                                            {!!Form::close()!!--}}
                        {{ \App\PurchaseOrder::orderStatus($result->account_status) }}
                    </td>
                    <td>
                      ...
                      @if($result->account_status!=4)

                       {!! Html::decode(link_to_route('employee.poPDF','<i class="fas fa-file-pdf"></i>PDF',$result->order_id,['class'=>'btn btn-app btn-edit','title'=>'PDF'])) !!}

                      	{!! Html::decode(link_to_route('employee.editPO','<i class="fas fa-edit"></i>Edit',[$result->order_id,$currentPage],['class'=>'btn btn-app btn-edit','title'=>'Edit'])) !!}
