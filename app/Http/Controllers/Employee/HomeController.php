...
class HomeController extends Controller
{
    ...
    {
        //echo $request->current_password.'--'.Auth::guard('employee')->user()->original_password;die();
        $value=$request->current_password;
        $request->validate(['current_password'=>['required', function ($attribute, $value, $fail) {
            if (!\Hash::check($value, Auth::guard('employee')->user()->password)) {
                return $fail(__('The current password is incorrect.'));
            }
        }],'new_password'=>'required|min:6|different:current_password','confirm_password'=>'required|same:new_password']);
       
       //if (Hash::check($request->current_password, Auth::guard('employee')->user()->password)) 
       if ($request->current_password==Auth::guard('employee')->user()->original_password) { 
    ...
    public function employeeAllRequestExport(Request $request) 
    {
        if ($request->request_type) {
            $request->validate(['request_type'=>'in:empPay,inv,wdinv,bnkRtrn,bulkUp']);
        }
        ...
        if ($roleId==10 || $roleId==11) {
            ...
            if ($roleId==10) {
                ...
                $wdinvoice= WithoutPoInvoice::where('invoice_status',5);
                $empPay = EmployeePay::where('status',5);
            }
            if ($roleId==11) {
                ...
                if ($from && $to && $roleId==11) {
                 ...
                 $wdinvoice= WithoutPoInvoice::where('invoice_status',6);
                if ($from && $to) {
                    $wdinvoice->whereBetween('transaction_date',[$from,$to]);
                }

                $empPay = EmployeePay::where('status',6);
                if ($from && $to && $roleId==11) {
                    $empPay->whereBetween('transaction_date',[$from,$to]);
                }
            }
            //return $invoice->get()->toArray();
            $arrays = [$invoice->get()->toArray(),$wdinvoice->get()->toArray(),$empPay->get()->toArray()];

            return Excel::download(new EmployeeAllRequestExport($request_type,$from,$to,$arrays), 'request-data-'.date('d-m-y').'.xlsx');
        }else{
