...
class EmployeeAllRequestExport implements FromCollection, WithMapping, WithHeadings
{
    ...
    */
    function __construct($request_type=null,$from='',$to='')
    {
    ...
    public function collection()
    {
        ...
        if ($this->request_type=='bulkUp') {
            if ($roleId==10) {
                $invoice = BulkCsvUpload::join('bulk_uploads', 'bulk_uploads.id', '=', 'bulk_csv_uploads.bulk_upload_id')->select('bulk_uploads.*','bulk_csv_uploads.*','bulk_csv_uploads.order_id as csv_order_id','bulk_uploads.order_id as bu_order_id','transaction_date')->where('bulk_uploads.status',5);
            }
            if ($roleId==11) {
                $invoice = BulkCsvUpload::join('bulk_uploads', 'bulk_uploads.id', '=', 'bulk_csv_uploads.bulk_upload_id')->select('bulk_uploads.*','bulk_csv_uploads.*','bulk_csv_uploads.order_id as csv_order_id','bulk_uploads.order_id as bu_order_id','transaction_date')->where('bulk_uploads.status',6);
                if ($from && $to && $roleId==11) {
                    $invoice->whereBetween('transaction_date',[$from,$to]);
                }
            }
